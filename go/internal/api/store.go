...
type Session struct {
	Status           string
	Output           string
	PendingInterrupt *agent.PendingInterrupt
	RunInput         agent.RunInput
	History          []string
	ToolLog          []agent.ToolCallRecord
}
...
func (s *Store) Create(runID string, input agent.RunInput) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// "completed").
func (s *Store) SetResult(runID string, result agent.RunResult) {
	s.mu.Lock()
	defer s.mu.Unlock()
	sess, ok := s.sessions[runID]
	if !ok {
...
func (s *Store) ToolLog(runID string) ([]agent.ToolCallRecord, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	sess, ok := s.sessions[runID]
	if !ok {
	...
	return append([]agent.ToolCallRecord(nil), sess.ToolLog...), true
}
