...
func (w *Worker) Consume(ctx context.Context, orchestrator *agent.Orchestrator) error {
...
func (w *Worker) handle(ctx context.Context, orchestrator *agent.Orchestrator, d amqp.Delivery) {
	var task AgentTask
...
func runToCompletion(ctx context.Context, orchestrator *agent.Orchestrator, task AgentTask) (agent.RunResult, error) {
	...
	for i := 0; err == nil && result.Interrupted && i < maxAutoApprovals; i++ {
		result, err = orchestrator.Resume(ctx, input, agent.Decision{Type: "approve"})
	}
	return result, err
}

func rawOutput(result agent.RunResult) string {
